...
class ModelArguments:
    ...
    previous_lora_path: Optional[str] = field(
        default=None,
        metadata={"help": "the path to load previous lora."}
    )
    
    previous_lora_grad_path: Optional[str] = field(
        default=None,
        metadata={"help": "the path to load previous lora grad."}
    )
...
def main():
    ...
    if model_args.previous_lora_path:
        ...
        for i, path in enumerate(previous_lora_list):   
            ...
            previous_grad[i] = torch.load(os.path.join(path, "lora_grad.pt"))
    ...
    for name, param in model.named_parameters():
        ...
        if ("lora" in name and "previous_lora_weights" not in name) or "trans_input" in name or "prompt_key" in name: # if "lora" in name or "trans_input" in name or "prompt_key" in name # if "lora_B" in name or "trans_input" in name or "prompt_key" in name:
            param.requires_grad = True
    ...
    if training_args.do_train:
        ...
        if is_main_process:
            ...
            torch.save(trainer.cur_grad, os.path.join(save_path, 'lora_grad.pt'))
