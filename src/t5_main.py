...
from CaLoRA_cl_trainer import CaLora_Trainer, DenserEvalCallback, skip_instructions
...
class ModelArguments:
    ...
    previous_lora_path: Optional[str] = field(
        default=None,
        metadata={"help": "the path to load previous loras."}
    )

    # 存储encoder的prompt_key
...
# call main function multiple times in 'your_job_name.sh', so that achieving continuous learning
def main():
    ...
    # load previous tasks’ LoRA
    if model_args.previous_lora_path:
        previous_lora_list = model_args.previous_lora_path.split(',')
        # lzy's Q: why reverse
        # lzy's A: reverse之后最近的task有最小的索引
        previous_lora_list.reverse() 

        previous_lora_num = len(previous_lora_list)
        print("the num of previous lora list:", previous_lora_num)
    ...
    for name, param in model.named_parameters():
        # freeze LMs‘ parameters
        param.requires_grad = False
        # set current task‘s lora mode = train
        # previous_lora_weight is appended in 'model_name'_prompt.py
        #if ("lora" in name and "previous_lora_weights" not in name) or "trans_input" in name or "prompt_key" in name:
        #if ("lora_B" in name and "previous_lora_weights" not in name) or "trans_input" in name or "prompt_key" in name:
        if "lora" in name or "trans_input" in name or "prompt_key" in name:
            param.requires_grad = True

    '''
    if previous_lora_num > 0:
        previous_lora_A, previous_lora_B = {}, {}

        for i in range(previous_lora_num):
            pre_lora_A, pre_lora_B = [], []

            for name, param in model.named_parameters():
                if f"previous_lora_weights_q.{i}.lora_A" in name or f"previous_lora_weights_v.{i}.lora_A" in name:
                    pre_lora_A.append(param.clone())
                
                if f"previous_lora_weights_q.{i}.lora_B" in name or f"previous_lora_weights_v.{i}.lora_B" in name:
                    pre_lora_B.append(param.clone())

            if pre_lora_A and pre_lora_B:  # 确保列表不为空
                previous_lora_A[i], previous_lora_B[i] = torch.stack(pre_lora_A), torch.stack(pre_lora_B)
            else:
                print(f"Warning: No parameters found for index {i}")

    print("previous_lora_A:", previous_lora_A[0].shape)
    print("previous_lora_B:", previous_lora_B[0].shape)
    '''
    ...
    # Attentive Reflection Module中提到的合成data(replay data)和attention weight(replay label)
    replay_dataset_dict, replay_label_dict = None, None
    # replay data和label的加载
    data_collator_replay = None
    if training_args.data_replay_freq != -1:
        # 先generate pseudo data再train主程序
        # detail generated process could be found in "pseudo_data/src"
        ...
        if model_args.load_checkpoint_from:
            ...
            for idx in range(cur_task_id):
                raw_datasets_gen = load_dataset(
                    os.path.join(CURRENT_DIR, "cl_dataset.py"),
                    data_dir=data_dir, # data_dir = data_args.gen_data_dir
                    download_config=download_config,
            ...
            # replay_dataset_dict存储生成数据；replay_label_dict存储attention
            replay_label_dict = {}
            for idx in range(cur_task_id):
            ...
            # replay_label_dict保存的是attention_weight
